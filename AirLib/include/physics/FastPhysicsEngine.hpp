...
private:
    ...
    void updatePhysics(PhysicsBody& body)
    {
        ...
        if(!body.physicsDisabled()) {
            ...
            Wrench next_wrench, next_wind_wrench;
            ...
            getNextKinematicsNoCollision(dt, body, current, next, next_wrench, wind_, next_wind_wrench);
            ...
            body.setWindWrench(next_wind_wrench);
    ...
    static std::pair<Wrench, Wrench> getDragWrench(const PhysicsBody& body, const Quaternionr& orientation,
        const Vector3r& linear_vel, const Vector3r& angular_vel_body, const Vector3r& wind_world)
    {
        ...
        // Wrench due to wind
        Wrench wind_wrench = Wrench::zero();
        ...
        const Vector3r wind_vel_body = VectorMath::transformToBodyFrame(-wind_world, orientation);
        ...
        for (uint vi = 0; vi < body.dragVertexCount(); ++vi) {
            ...
            const Vector3r vel_vertex_wind = wind_vel_body;
            const real_T vel_comp_wind = vertex.getNormal().dot(vel_vertex_wind);
            ...
            if (vel_comp > kDragMinVelocity) {
                ...
                const Vector3r drag_force_wind = vertex.getNormal() * (-vertex.getDragFactor() * air_density * vel_comp_wind * vel_comp_wind);
                const Vector3r drag_torque_wind = vertex.getPosition().cross(drag_force_wind);

                wrench.force += drag_force;
                wrench.torque += drag_torque;

                wind_wrench.force += drag_force_wind;
                wind_wrench.torque += drag_torque_wind;
            }
        }
        ...
        wind_wrench.force = VectorMath::transformToWorldFrame(wind_wrench.force, orientation);

        return std::pair<Wrench, Wrench>(wrench, wind_wrench);
    }
    ...
    static void getNextKinematicsNoCollision(TTimeDelta dt, PhysicsBody& body, const Kinematics::State& current,
        Kinematics::State& next, Wrench& next_wrench, const Vector3r& wind, Wrench& next_wind_wrench)
    {
        const real_T dt_real = static_cast<real_T>(dt);
        ...
        if (body.isGrounded()) {
            ...
            std::pair<Wrench, Wrench> drag_wrenches = getDragWrench(body, current.pose.orientation, avg_linear, avg_angular, wind);
            const Wrench drag_wrench = drag_wrenches.first;

            next_wrench = body_wrench + drag_wrench;

            next_wind_wrench = drag_wrenches.second;
